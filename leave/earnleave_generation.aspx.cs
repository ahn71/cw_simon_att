    ...
    public partial class earnleave_generation : System.Web.UI.Page
    {
        ...
        private void loadGenerationList()
        {
            try
            {
                ...
                sqlCmd = "select distinct CompanyID,convert(varchar(10),GenerateDate,102) as GenerateDate, format(GenerateDate,'MMM-yyyy') as GenerateMonth from Earnleave_BalanceDetailsLog where CompanyID='" + CompanyID + "' order by GenerateDate desc";
        ...
        private void generateEarnleave()
        {
            try
            {
                ...
                sqlCmd = "select convert(varchar(10),ELStartDate,120) as ELStartDate,Status,DurationDays from Earnleave_Setting";
                sqlDB.fillDataTable(sqlCmd, dt = new DataTable());
                lblErrorMsg.Text += ",76";
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (EmpID == "")
                        sqlCmd = "select distinct EmpId,convert(varchar(10),EmpJoiningDate,120) as EmpJoiningDate  from v_tblAttendanceRecord where    PaybleDays=1 and CompanyId = '" + CompanyID + "' and  ATTDate>= '" + generateDate.Substring(0, 7) + "-01" + "' and ATTDate<= '"+ generateDate + "' ";
                    else
                        sqlCmd = "select distinct EmpId,convert(varchar(10),EmpJoiningDate,120) as EmpJoiningDate  from v_tblAttendanceRecord where    PaybleDays=1 and CompanyId = '" + CompanyID + "' and  ATTDate>= '" + generateDate.Substring(0, 7) + "-01" + "' and ATTDate<= '" + generateDate + "' and EmpID='"+ EmpID + "'";
                   ...
                   DataTable   dtEmp = new DataTable();
                    ...
                    if (dtEmp != null && dtEmp.Rows.Count > 0)
                        for (int i=0; i < dtEmp.Rows.Count; i++)
                        {
                            ...
                            sqlCmd = "select convert(varchar(10),ATTDate,120) as ATTDate  from tblAttendanceRecord where PaybleDays=1 and EmpID='"+ EmpID + "'  and ATTDate>= '"+ StartDate + "' and ATTDate<= '"+generateDate+"' order by convert(varchar(10), ATTDate, 120)";
                            ...
                            if (dtAttRecords!=null && dtAttRecords.Rows.Count>= DurationDays)
                            {
                                ...
                                sqlCmd = "INSERT INTO [dbo].[Earnleave_BalanceDetailsLog] ([CompanyID],[EmpID],[EarnLeaveLastDate],[GenerateDate],[EarnLeaveDays],[EntryDatetime])" +
        ...
        {
            try {
                sqlCmd = " select  ISNULL(convert(varchar(10), DATEADD(DAY,1,max(EarnLeaveLastDate)),120),'" + InitialDate + "') as EarnLeaveLastDate from Earnleave_BalanceDetailsLog where  EmpID='" + EmpID + "'  and EarnLeaveLastDate<= '" + GenerateDate + "'";
                dt = new DataTable();
        ...
        private void saveEarnleaveMonthlyInformation(string CompanyID, string Status, string Month,string ToDate,string EmpID, string _StartDate, string _LastDateOfEL)
        {
            try {
                ...
                //---get present month's days ---
                    sqlCmd = "select EmpId,count(EmpId) as P  from v_tblAttendanceRecord where IsActive=1 and CompanyID='" + CompanyID + "'  and ATTStatus in(" + Status + ") and PaybleDays=1 and ATTDate>=(case when EmpJoiningDate>'" + Month + "' then EmpJoiningDate else '" + Month + "' end) and ATTDate<='" + ToDate + "' and EmpJoiningDate<'" + ToDate + "' and EmpID='" + EmpID + "' group by EmpId";
                dt = new DataTable();
                dt=CRUD.ExecuteReturnDataTable(sqlCmd,sqlDB.connection);               
                if (dt != null && dt.Rows.Count > 0)
                {
                ...
                sqlCmd = "select EmpId,count(EmpId) as P  from v_tblAttendanceRecord where IsActive=1 and CompanyID='" + CompanyID + "'  and ATTStatus in(" + Status + ") and PaybleDays=1 and ATTDate>=(case when EmpJoiningDate>'" +DateTime.Parse( _LastDateOfEL).AddDays(1).ToString("yyyy-MM-dd") + "' then EmpJoiningDate else '" + DateTime.Parse(_LastDateOfEL).AddDays(1).ToString("yyyy-MM-dd") + "' end) and ATTDate<='" + ToDate + "' and EmpJoiningDate<'" + ToDate + "' and EmpID='" + EmpID + "' group by EmpId";
                dt = new DataTable();
                dt = CRUD.ExecuteReturnDataTable(sqlCmd, sqlDB.connection);
                    if (dt != null && dt.Rows.Count > 0)
                    {
                ...
                //---end previous month's days for this month ---
                sqlCmd = "INSERT INTO [dbo].[Earnleave_MonthlyInfo] ([CompanyID],[EmpID],[Month],[PresentDays],[EntryTime],[PreMonthDays],[NextMonthDays])" +
        ...
        private bool deleteEarnleaveMonthlyInformation(string CompanyID, string Month,string EmpID)
        {
            ...
            {
                if(EmpID=="")
                sqlCmd = "delete Earnleave_MonthlyInfo where CompanyID='"+ CompanyID + "' and Month='" + Month + "'";
                else
                    sqlCmd = "delete Earnleave_MonthlyInfo where CompanyID='" + CompanyID + "'  and Month='" + Month + "' and EmpID='" + EmpID + "'";
        ...
        private bool deleteEarnleaveBalance(string Month, string EmpID)
        {
            try
            {
                    sqlCmd = "delete Earnleave_BalanceDetailsLog where EmpID='"+ EmpID + "' and GenerateDate='"+ Month + "'";
        ...
        private bool deleteExEarnLeave(string CompanyID, string GenerateDate)
        {
            try
            {
                sqlCmd = "delete Earnleave_BalanceDetailsLog where CompanyID='"+ CompanyID + "' and GenerateDate='" + GenerateDate + "'";
                return CRUD.Execute(sqlCmd, sqlDB.connection);
